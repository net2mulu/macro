...
import { BaseProject, Category } from '@/lib/interfaces' 
...
    return projects.map((p) => {
        // Handle cases where 'p.category' might be a string OR an object (e.g., Strapi relation object)
      const categoryValue =
...
        .map((c: Category) => c.name) // <--- CRITICAL FIX: Extract the string 'name'
        .filter(Boolean)
      return ['All Projects', ...names]
    }
    
    // Fallback to project-derived categories from normalized data
...
      )
    )
    return ['All Projects', ...dynamic]
  }, [normalizedProjects, categoriesData])
